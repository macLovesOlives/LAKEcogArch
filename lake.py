...
def actionName2action(actionName):
    '''takes in a name and returns the action object

    actionName (string) is the name of the action.'''

    # usage: actionName2action('actionTalkNormal')
    for theAction in actions:
        if theAction.name == actionName:
            return theAction
# end def actionName2action
...
def goal2nextAction(goalName):
    ''' takes in a goalName and outputs the next action to be taken.

    goalName is the ID of the goal.'''
## DO NOT USE. THIS FUNCTION HAS BEEN REPLACED WITH A METHOD IN the GOAL class
    ## ??? delete this after removing all references to it
    
    # usage: goal2nextAction('goalWalkFast')
    #DEBUGGINGprint("length of actions is", len(actions))
    
    ## loop through all the actions, return the action that matches the goal 
    for action in actions:
        if action.goalName == goalName:
            return action.name
            #DEBUGGING print "The action is", action.name, "."
# end def goal2nextAction
